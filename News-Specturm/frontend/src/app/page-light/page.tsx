...
export default function HomeLightTheme() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-900">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-56" : "w-12"} shrink-0 border-r border-gray-200 bg-white flex flex-col transition-all duration-300`}
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 pt-3 pb-1">
          {sidebarOpen && (
            <div className="flex items-center gap-2 pl-1">
              ...
              <span className="text-sm font-semibold tracking-tight text-gray-800">BiasDetect</span>
            </div>
          )}
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="p-2 rounded-md hover:bg-gray-100 transition-colors text-gray-400 hover:text-gray-700 ml-auto"
        ...
        <div className={`${sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity duration-300 px-3 pt-4 pb-4`}>
          <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-400 px-2 mb-2">Categories</p>
        ...
        {/* Bottom section */}
        <div className={`${sidebarOpen ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity duration-300 mt-auto border-t border-gray-200 p-3`}>
          <div className="flex items-center gap-3 rounded-md px-2.5 py-2 text-sm text-gray-500 hover:bg-gray-50 transition-colors cursor-pointer">
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center text-xs text-gray-600 font-medium shrink-0">
              G
            </div>
            <div className="min-w-0">
              <p className="text-xs text-gray-700 whitespace-nowrap font-medium">Guest User</p>
              <p className="text-[10px] text-gray-400 whitespace-nowrap">Not signed in</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50">
        {/* Top bar */}
        <header className="px-6 py-3.5 flex items-center gap-4 shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-sm"
          ...
          <div className="flex-1 flex justify-center">
            <div className="relative w-full max-w-2xl">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 bg-gray-50 pl-9 pr-4 py-2 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-200 focus:bg-white transition-all"
              />
            </div>
          </div>
          <Link
            href="/"
            className="shrink-0 text-xs text-gray-400 hover:text-gray-600 transition-colors hidden sm:block"
          >
            Dark version
          </Link>
          <Link
        ...
        <section className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Stay informed. Detect bias.</p>
            </div>
            <span className="text-[10px] font-medium text-gray-400 uppercase tracking-wider">{articles.length} articles</span>
          </div>

          {/* Featured card */}
          <div className="mb-5 rounded-2xl border border-blue-100 bg-white overflow-hidden transition-all cursor-pointer group"
            style={{boxShadow: "0 2px 12px 0 rgba(59,130,246,0.08), 0 1px 3px 0 rgba(0,0,0,0.06)"}}>
            <div className="h-52 bg-gradient-to-br from-blue-50 via-indigo-50/50 to-white relative flex flex-col justify-end p-5">
              {/* Decorative dots */}
              <div className="absolute inset-0 opacity-[0.06]" style={{backgroundImage: "radial-gradient(circle, #3b82f6 1px, transparent 1px)", backgroundSize: "24px 24px"}} />
              <div className="absolute top-4 right-4 flex items-center gap-1.5 bg-white/80 backdrop-blur-sm rounded-full px-3 py-1 border border-blue-100">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                <span className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Featured</span>
              </div>
              <div className="relative">
                <div className="h-2.5 w-24 bg-gray-200 rounded mb-2" />
                <div className="h-4 w-64 bg-gray-200/80 rounded mb-1.5" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {articles.slice(1).map((article, i) => (
              <div
                key={article.id}
                className={`rounded-xl border ${cardBorders[i % cardBorders.length]} bg-white overflow-hidden transition-all hover:-translate-y-0.5 cursor-pointer group`}
              ...
              >
                ...
                <div className="p-3.5">
                  <div className="h-2 w-14 bg-gray-100 rounded mb-2" />
                  <div className="h-2.5 w-full bg-gray-100 rounded mb-1.5" />
                  <div className="h-2.5 w-4/5 bg-gray-100/80 rounded" />
